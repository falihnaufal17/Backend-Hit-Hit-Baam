...
module.exports = {
    getAllScore: () => {
        return new Promise((resolve, reject) => {
            conn.query('SELECT skor.idskor, user.iduser, user.fullname, user.image, skor.skor, skor.created_at, skor.updated_at FROM skor INNER JOIN user ON skor.iduser = user.iduser ORDER BY skor.skor DESC LIMIT 10 OFFSET 0', (err, result) => {
                if (!err) {
                    resolve(result)
                } else {
